...
exports.modifyCollectionColumn = function (db, name, column, cb) {
	...
	queue.add(function (cb) {
		db.query("ALTER TABLE " + name + " ALTER " + col_name + " TYPE " + col_type, cb);
	});

	if (column) {
		if (column.match(/NOT NULL/)) {
			queue.add(function (cb) {
				db.query("ALTER TABLE " + name + " ALTER " + col_name + " SET NOT NULL", cb);
			});
		} else {
			queue.add(function (cb) {
				db.query("ALTER TABLE " + name + " ALTER " + col_name + " DROP NOT NULL", cb);
			});
		}

		if (m = column.match(/DEFAULT (.+)$/)) {
			queue.add(function (cb) {
				db.query("ALTER TABLE " + name + " ALTER " + col_name + " SET DEFAULT " + m[1], cb);
			});
		}
	}
...
function checkColumnTypes(db, collection, columns, cb) {
	...
	for (var k in columns) {
		if (columns[k].type == "enum") {
			queue.add(k, columns[k], function (name, col, cb) {
